...
class MusicTransformer(nn.Module):
    ...
    def __init__(self, n_layers=6, num_heads=8, d_model=512, dim_feedforward=1024,
        ...
        # RPR Transformer
            ...
            encoder = TransformerEncoderRPR(
                encoder_layer, self.nlayers, encoder_norm)
            self.transformer = nn.Transformer(
                d_model=self.d_model, nhead=self.nhead, num_encoder_layers=self.nlayers,
                num_decoder_layers=0, dropout=self.dropout,  # activation=self.ff_activ,
    ...
    def generate(self, primer=None, target_seq_length=1024, beam=0, beam_chance=1.0):
        ...
        gen_seq = torch.full((1, target_seq_length), TOKEN_PAD,
                             dtype=TORCH_LABEL_TYPE, device=get_device())

        num_primer = len(primer)
        gen_seq[..., :num_primer] = primer.type(
            TORCH_LABEL_TYPE).to(get_device())
        ...
        while(cur_i < target_seq_length):
            ...
            if(beam_ran <= beam_chance):
                ...
                if(next_token == TOKEN_END):
                    print("Model called end of sequence at:",
                          cur_i, "/", target_seq_length)
                    break
